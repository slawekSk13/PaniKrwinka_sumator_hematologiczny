...
const TableMarkup = ({patient, results, progress, calcFinished}) => {
    ...
    return (<ColorTheme.Consumer>
            {colors => <StyledTable colors={colors}>
                <colgroup>
                    <col/>
                    <col/>
                    <col/>
                ...
                {renderResults && <>
                    ...
                    {types.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td>
                                <strong>{(results.leukogram.relative[item] * (results.leukogram.wbc.corrected === 'b.d.' ? results.leukogram.wbc.nominal : results.leukogram.wbc.corrected) / progress).toFixed(2)}</strong> G/l
                            </td>
                        </tr>
                    ))}
                    <tr>
